...
import com.ymc.paper.domain.DocumentContentAsset;
...
public class DocumentContentIngestService {
    ...
    public void ingest(UUID documentId, String manifestKey) {
        ...
        assetRepository.saveAll(pkg.assets().stream()
                .map(a -> DocumentContentAsset.of(documentId, a.assetKey(), a.s3Key(), a.mediaType()))
                .toList());
    }

    @Transactional(readOnly = true)
    public boolean isIngested(UUID documentId) {
        return contentRepository.existsById(documentId);
    }
}
