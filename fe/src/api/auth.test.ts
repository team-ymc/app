...
describe('auth.js', () => {
  ...
  it('bootstrap: refresh 성공 → user 반환, 이후 요청에 Bearer 부착', async () => {
    globalThis.fetch = vi.fn()
      ...
      .mockResolvedValueOnce(jsonRes(200)) as unknown as typeof fetch;
    ...
    expect(globalThis.fetch).toHaveBeenLastCalledWith('/api/papers',
      expect.objectContaining({ headers: { Authorization: 'Bearer A1' } }));
  });

  it('bootstrap: 401(쿠키 없음) → null', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(jsonRes(401)) as unknown as typeof fetch;
  ...
  it('authFetch: 401 → refresh 1회 → 원 요청 재시도 성공', async () => {
    globalThis.fetch = vi.fn()
      ...
      .mockResolvedValueOnce(jsonRes(200, { papers: [] })) as unknown as typeof fetch; // 재시도
    const res = await authFetch('/api/papers');
    expect(res.status).toBe(200);
    expect(globalThis.fetch).toHaveBeenCalledTimes(3);
    expect((globalThis.fetch as ReturnType<typeof vi.fn>).mock.calls[1][0]).toBe('/api/auth/refresh');
  });
  ...
  it('logout: POST 후 Authorization 미부착', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(jsonRes(200, TOKEN_BODY)) as unknown as typeof fetch;
    await bootstrap();
    globalThis.fetch = vi.fn().mockResolvedValue(jsonRes(204)) as unknown as typeof fetch;
    await logout();
    expect(globalThis.fetch).toHaveBeenCalledWith('/api/auth/logout', { method: 'POST' });

    globalThis.fetch = vi.fn().mockResolvedValue(jsonRes(200)) as unknown as typeof fetch;
    await authFetch('/api/papers');
    expect(globalThis.fetch).toHaveBeenLastCalledWith('/api/papers',
