...
export interface StreamOpts {
  ...
  content: string;
  selections?: SelectionAnchors[] | null;
  signal?: AbortSignal;
 ...
 */
export async function streamChatMessage({ paperId, sessionId, clientMessageId, content, selections, signal, onEvent }: StreamOpts): Promise<void> {
  const body: { clientMessageId: string; content: string; sessionId?: string; selections?: SelectionAnchors[] } = { clientMessageId, content };
  if (sessionId) body.sessionId = sessionId;
  if (selections && selections.length > 0) body.selections = selections; // 빈 배열은 계약 위반(minItems 1) — 생략
